...
- `r` 表示运行队列(就是说多少个进程真的分配到`CPU`)，我测试的服务器目前`CPU`比较空闲，没什么程序在跑，当这个值超过了`CPU`数目，就会出现`CPU`瓶颈了。这个也和`top`的负载有关系，一般负载超过了3就比较高，超过了5就高，超过了10就不正常了，服务器的状态很危险。top的负载类似每秒的运行队列。如果运行队列过大，表示你的`CPU`很繁忙，一般会造成`CPU`使用率很高。
